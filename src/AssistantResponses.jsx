    ...
    if (inputAble) {
      if (messages.length > 4　&& messages[messages.length - 1].role == "user") {
        const makeResponse = async () => {
              try {
                const chatMessages = [
                  { role: "system", content: `あなたは心理カウンセラーです。ユーザーとの会話でオープンダイアローグの原則に従って、共感的かつ受容的な姿勢で対話を行ってください。質問を通じてユーザーが自身の感情や問題に気づけるように導いてください。ユーザーの発言に対して深い洞察を示し、次の質問や確認のための発言を行ってください。` },
                    ...messages.map(message => ({...message, role: "user"}))
                ];
                let response = await openai.chat.completions.create({
                  model: "gpt-4o",
        ...
        if (messages.length > 4 && messages[messages.length - 2].role == "user") {
            const makeResponse = async () => {
                for(let i = 0; i < names.length * reflectChatCount; i++){
                    ...
                    try {
                        const reflectMessages = [
                            { role: "system", content: `あなたは心理カウンセリングにおいて、リフレクティング（反射的傾聴）を行っています。ユーザーがこれまで話してきた内容を振り返り、3人の治療者として、それぞれ異なる観点から感じたことや考えたことを述べてください。リフレクティングは、ユーザーの発言を整理し、理解を深め、内省を促すことを目的としています。3人の治療者が順番にリフレクティングを行い、最終的にユーザーが話したことに対して丁寧にまとめ、今後の展開に繋がる発言をしてください。` },
                            ...messages.map(message => ({ ...message, role: "user" }))
                        ];
                        let response = await openai.chat.completions.create({
                          model: "gpt-4o",
